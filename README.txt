...
DELETE  /api/user?userid={id}                                           -delete user by id

GET     /api/user/login?username={username}&password={password}         -verify user login, if successful, it returns user object, if not returns 400 error message
...
DELETE  /api/workouts?workoutid={id}                                    -delete workout by id
